...
<body class="flex items-center justify-center min-h-screen">
    ...
    <script type="module">
            ...
            return (
                <div className="absolute inset-0 bg-gray-50 z-50 flex flex-col animate-in slide-in-from-right duration-300">
                    <div className="h-14 bg-white border-b flex items-center px-4 shrink-0">
                        ...
                        <span className="ml-2 font-bold">系统设置</span>
                    </div>
                    <div className="p-4 space-y-4 overflow-y-auto">
                        <div className="bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm">
                            <button onClick={handleExport} className="w-full flex items-center p-4 active:bg-gray-50 border-b">
                                ...
                                <div className="text-left"><p className="font-medium text-sm">导出备份</p><p className="text-xs text-gray-400">下载聊天记录到本地</p></div>
                            </button>
                            <button onClick={() => fileRef.current.click()} className="w-full flex items-center p-4 active:bg-gray-50 border-b">
                                ...
                                <div className="text-left"><p className="font-medium text-sm">导入备份</p><p className="text-xs text-gray-400">从 JSON 文件恢复数据</p></div>
                            ...
                            <button onClick={clearData} className="w-full flex items-center p-4 active:bg-gray-50 text-red-500">
                                ...
                                <p className="font-medium text-sm">清空所有数据</p>
                            </button>
                        </div>
                        <div className="p-4 text-center">
                            <ShieldCheck className="mx-auto text-gray-300 mb-2" size={32}/>
                            <p className="text-[10px] text-gray-400 uppercase tracking-widest">QQ Lite OS v1.1.0 • Secure Storage</p>
                        </div>
                    </div>
                </div>
            );
            ...
            return (
                <div className="absolute inset-0 bg-[#f2f2f7] z-40 flex flex-col animate-in slide-in-from-right">
                    <div className="h-14 bg-white/80 backdrop-blur-md border-b flex items-center px-4 shrink-0">
                        ...
                        <div className="ml-3"><p className="font-bold text-sm">{contact.name}</p><p className="text-[10px] text-green-500">正在运行</p></div>
                    </div>
                    ...
                    <div className="p-3 bg-white border-t flex items-center space-x-2 pb-safe">
                        <input type="file" ref={fileRef} className="hidden" accept="image/*" onChange={async(e) => {
                            ...
                            onSendMessage({ text: '[图片]', image: b64, isSelf: true });
                        }} />
                        <button onClick={() => fileRef.current.click()}><ImageIcon className="text-gray-400" /></button>
                        <input className="flex-1 bg-gray-100 rounded-full px-4 py-2 text-sm outline-none" placeholder="输入消息..." value={text} onChange={e=>setText(e.target.value)} onKeyDown={e=>e.key==='Enter' && (onSendMessage({text, isSelf:true}), setText(''))} />
        ...
        const App = () => {
            ...
            const [sessions, setSessions] = useState(() => JSON.parse(localStorage.getItem('QQ_LITE_SESSIONS')) || [{ id: '1', name: 'Gemini Assistant', avatar: '🤖', lastMessage: '你好！我是你的 AI 助手。' }]);
            const [allMessages, setAllMessages] = useState(() => JSON.parse(localStorage.getItem('QQ_LITE_MSGS')) || { '1': [{ text: '你好！我是你的 AI 助手，你可以通过设置导出我的记忆。', isSelf: false }] });
